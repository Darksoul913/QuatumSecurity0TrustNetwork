...
ABLATIONS = [
    ...
    Ablation("A1", "QBER features", enable_qber=False),
    Ablation("A2", "ML engine", enable_ml=False),
    Ablation("A3", "G2 tamper signaling", enable_g2=False),
    Ablation("A4", "Adaptive enforcement", enable_adaptive=False),
    Ablation("A5", "QKD", enable_qkd=False),
    Ablation("A6", "All quantum layers", enable_qkd=False, enable_qber=False, enable_g2=False),
]
...
def qber_features(scenario: str, enable_qber: bool, rng: np.random.Generator) -> tuple[float, float, float]:
    if not enable_qber:
        return 0.0, 0.0, 0.0
    if scenario == "attack":
    ...
    return min(1.0, mean / 0.30), min(1.0, var / 0.02), min(1.0, crossings / 20.0)


def tamper_signal(scenario: str, enable_g2: bool, rng: np.random.Generator, k_ancilla: int) -> int:
    if not enable_g2:
        return 0
    if scenario == "attack":
...
def policy_decision(risk_score: int, g2_signal: int, ablation: Ablation) -> bool:
    if not ablation.enable_adaptive:
        return True
    if g2_signal:
        return False
    if ablation.enable_ml and risk_score >= 2:
        return False
    return True
...
def run(trials: int, k_ancilla: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for ablation in ABLATIONS:
        for _ in range(trials):
            ...
            features = X[idx].copy()
            features[2:] = qber_features(scenario, ablation.enable_qber, rng)

            start = time.perf_counter()
            risk_score = int(model.predict(features.reshape(1, -1))[0]) if ablation.enable_ml else 0
            g2 = tamper_signal(scenario, ablation.enable_g2, rng, k_ancilla)
            allow = policy_decision(risk_score, g2, ablation)
            latency_ms = (time.perf_counter() - start) * 1000.0

            attack_present = scenario == "attack"
            key_agreement = ablation.enable_qkd and (not attack_present or rng.random() > 0.25)
            decryption_success = allow and (key_agreement or not ablation.enable_qkd)
            ...
            rows.append(
                {
                    ...
                    "enable_qkd": ablation.enable_qkd,
                    "enable_qber": ablation.enable_qber,
                    "enable_ml": ablation.enable_ml,
                    "enable_g2": ablation.enable_g2,
                    "enable_adaptive": ablation.enable_adaptive,
                }
            )
